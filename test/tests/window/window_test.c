...
static void window_move_callback(istd_window window, istd_vector2_i32 position) {
	istd_unused_parameter(window);
	istd_println("Window Moved: x -> %i, y -> %i", position.x, position.y);
}

static void window_size_callback(istd_window window, istd_vector2_i32 size) {
	istd_unused_parameter(window);
	istd_println("Window Sized: width -> %i, height -> %i", size.width, size.height);
}

static void framebuffer_size_callback(istd_window window, istd_vector2_i32 size) {
	istd_unused_parameter(window);
	istd_println("Framebuffer Sized: width -> %i, height -> %i", size.width, size.height);
}

static void window_iconified_callback(istd_window window, bool iconified) {
	istd_unused_parameter(window);
	istd_println("Window Iconified: %s", iconified == true ? "true" : "false");
}

static void window_maximized_callback(istd_window window, bool maximized) {
	istd_unused_parameter(window);
	istd_ignore_return(printf("Window Maximized: %s", maximized == true ? "true" : "false"));
}

static void mouse_move_callback(istd_window window, istd_vector2_i32 mouse_position) {
	istd_unused_parameter(window);
	istd_println("Mouse Moved: x -> %i, y -> %i", mouse_position.x, mouse_position.y);
}

static void mouse_scroll_callback(istd_window window, istd_vector2_f32 scroll_offset) {
	istd_unused_parameter(window);
...
static void mouse_button_callback(istd_window window, istd_mouse_button button, istd_key_modifier_flags mods, bool down) {
	istd_unused_parameter(window);
...
static void mouse_entered_callback(istd_window window, bool entered) {
	istd_unused_parameter(window);
	istd_println("Mouse %s", entered == true ? "Entered" : "Exited");
}

static void key_callback(istd_window window, istd_key key, istd_key_modifier_flags mods, bool down) {
	istd_unused_parameter(window);
...
static void char_callback(istd_window window, char32_t character, istd_key_modifier_flags mods) {
	istd_unused_parameter(window);
...
static void dpi_callback(istd_window window, istd_vector2_i32 dpi) {
	istd_unused_parameter(window);

	istd_println("New monitor dpi: x -> %d, y -> %d", dpi.x, dpi.y);
}

static void path_drop_callback(istd_window window, wchar_t** paths, size_t paths_size) {
	istd_unused_parameter(window);
...
istd_test_msg test_window(void) {
	istd_test_msg msg = { 0 };
    ...
    // istd_window_win32_set_style((istd_window_win32)window, ISTD_WINDOW_STYLE_DEFUALT_BIT);
	...
	istd_window_win32_set_icon((istd_window_win32)window, &image);
